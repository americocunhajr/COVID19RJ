...
if (init == 6) country = 'Colombia'; color = [96,209,224]/255; end  
if (init == 8) country = 'Argentina'; color = [255,130,113]/255; end
if (init == 10) country = 'Bolivia'; color = [209,227,105]/255; end
if (init == 14) country = 'Paraguay'; color = [0,104,44]/255; end
...
if (init == 11) country = 'Cuba'; color = [191,171,72]/255; end
if (init == 7) country = 'Dominican Republic'; color = [236,64,122]/255; end
if (init == 9) country = 'Panama'; color = [0.4,0.4,0.4]; end
...
hold on;
 ...
 fig=loglog(tot_deaths_pm,new_deaths7_pm,'DisplayName',[country,'  ',num2str(tot_deaths(max(size(tot_deaths)),1)),' mortes'],"color",color,'LineWidth', linew);
 hold on;
...
hold on;
 ...
 fig=loglog(tot_cases_pm,new_cases7_pm,'DisplayName',[country,'  ',num2str(tot_cases(max(size(tot_cases)),1)),' casos'],"color",color,'LineWidth', linew);
 hold on;
...
title({'Letalidade da epidemia',['América Latina em ',datestr(end_time,24)]},'FontSize',fonte_titulo);
...
set(gca,'XTickLabel',{'0.1','1','10','100','1k','10k'})
...
set(gca,'FontSize',fonte_padrao)
title({'Letalidade semanal da epidemia',['América Latina em ',datestr(end_time,24)]},'FontSize',fonte_titulo);
xlabel({['Dias desde que se ultrapassou ',num2str(X_deaths_pm),' morte'],'(por milhão de habitantes)'},'FontSize',fonte_labels);
ylabel ({'Novas mortes por semana','(por milhão de habitantes)'},'FontSize',fonte_labels);
legend ("location", "northeastoutside");
...
set(gca,'FontSize',fonte_padrao)
title({'Contágio semanal da epidemia',['América Latina em ',datestr(end_time,24)]},'FontSize',fonte_titulo);
xlabel({['Dias desde que se ultrapassou ',num2str(X_cases_pm),' caso'],'(por milhão de habitantes)'},'FontSize',fonte_labels);
ylabel ({'Novos casos por semana','(por milhão de habitantes)'},'FontSize',fonte_labels);
legend ("location", "northeastoutside");
y_init=0.1;
