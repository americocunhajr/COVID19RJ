...
% Extraindo dados
img_casos_diarios_aux = zeros(length(Estados),max_size);
img_casos_acumulados_aux = zeros(length(Estados),max_size);
img_obitos_diarios_aux = zeros(length(Estados),max_size);
img_obitos_acumulados_aux = zeros(length(Estados),max_size);

cases_old = zeros(length(Estados),max_size);
deaths_old = zeros(length(Estados),max_size);
dates = zeros(1,max_size);

img_casos_diarios = zeros(length(Regioes),max_size);
img_casos_acumulados = zeros(length(Regioes),max_size);
img_obitos_diarios = zeros(length(Regioes),max_size);
img_obitos_acumulados = zeros(length(Regioes),max_size);
...
for i=1:length(Estados)
    ...
    img_casos_diarios_aux = -cases_old + img_casos_acumulados_aux;
    img_obitos_diarios_aux = -deaths_old + img_obitos_acumulados_aux;
...
I_estado=find(arquivogeral.state == string("TOTAL"));
tabela = arquivogeral(I_estado,:);
dates = tabela.date;
end_date = dates(length(date));
...
set(hfonte,'Rotation',90,'color',[0,0,0],'horizontalAlignment', 'right','verticalAlignment', 'top','FontSize',7);
    ...
    x_indices_label = round([1:((size(dates,1))-1)/5:size(dates)])
    xticklabels({datestr(dates(x_indices_label),20); num2str(round([0.0 , max(img_max)/2, max(img_max)]'),6) });    
xtickangle(45)
...
if (t == 2) title({'Novas mortes por semana',['Regiões do Brasil em ',datestr(arquivogeral.date(length(arquivogeral.date)),24)]},'FontSize',12); end
if (t == 1) title({'Novos casos por semana',['Regiões do Brasil em ',datestr(arquivogeral.date(length(arquivogeral.date)),24)]},'FontSize',12); end
if (t == 4) title({'Mortes acumuladas',['Regiões do Brasil em ',datestr(arquivogeral.date(length(arquivogeral.date)),24)]},'FontSize',12); end
if (t == 3) title({'Casos acumulados',['Regiões do Brasil em ',datestr(arquivogeral.date(length(arquivogeral.date)),24)]},'FontSize',12); end
...
end_time = tabela.date(length(tabela.date));
