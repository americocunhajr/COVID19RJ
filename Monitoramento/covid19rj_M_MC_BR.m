...
for i=1:length(Estados)
    ...
    img_casos_acumulados(i,max_size-height(tabela_aux)+1:end) = tabela.totalCasesMS;
    img_obitos_acumulados(i,max_size-height(tabela_aux)+1:end) = tabela.deathsMS;
    
    cases_old = [zeros(27,1) , img_casos_acumulados(1:27,1:end-1)];
    deaths_old= [zeros(27,1) , img_obitos_acumulados(1:27,1:end-1)];
    img_casos_diarios = -cases_old + img_casos_acumulados;
    img_obitos_diarios = -deaths_old + img_obitos_acumulados;
...
if (t == 2) title({'Novas mortes por semana',['Brasil em ',datestr(arquivogeral.date(length(arquivogeral.date)),24)]},'FontSize',12); end
if (t == 1) title({'Novos casos por semana',['Brasil em ',datestr(arquivogeral.date(length(arquivogeral.date)),24)]},'FontSize',12); end
if (t == 4) title({'Mortes acumuladas',['Brasil em ',datestr(arquivogeral.date(length(arquivogeral.date)),24)]},'FontSize',12); end
if (t == 3) title({'Casos acumulados',['Brasil em ',datestr(arquivogeral.date(length(arquivogeral.date)),24)]},'FontSize',12); end
