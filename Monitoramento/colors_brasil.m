...
% Extraindo dados
img_casos_diarios = zeros(length(Estados),height(tabela)-indice_minimo+1);
img_casos_acumulados = zeros(length(Estados),height(tabela)-indice_minimo+1);
img_obitos_diarios = zeros(length(Estados),height(tabela)-indice_minimo+1);
img_obitos_acumulados = zeros(length(Estados),height(tabela)-indice_minimo+1);
...
for i=1:length(Estados)
    ...
    img_casos_diarios(i,:) = tabela.casosNovos;
    img_obitos_diarios(i,:) = tabela.obitosNovos;
    img_casos_acumulados(i,:) = tabela.casosAcumulados;
    img_obitos_acumulados(i,:) = tabela.obitosAcumulados;
end
...
if (t == 2) title({'Novas mortes por semana',['Brasil em ',datestr(tabela.data(length(tabela.data)),24)]},'FontSize',12); end
if (t == 1) title({'Novos casos por semana',['Brasil em ',datestr(tabela.data(length(tabela.data)),24)]},'FontSize',12); end
if (t == 4) title({'Mortes acumuladas',['Brasil em ',datestr(tabela.data(length(tabela.data)),24)]},'FontSize',12); end
if (t == 3) title({'Casos acumulados',['Brasil em ',datestr(tabela.data(length(tabela.data)),24)]},'FontSize',12); end
