...
%Fontes de dados
arquivogeral = readtable([pwd,'/Dados/arquivo_geral.csv']);
...
indice_minimo = height(arquivogeral)/length(Estados);
for i=1:length(Estados)
    I_estado=find(arquivogeral.estado == string(Estados(i,:)));
    tabela = arquivogeral(I_estado,:);
    I = find(tabela.casosAcumulados>= min_casos_acum_consider);
    indice_minimo = min(min([I;indice_minimo]));
end
...
% Extraindo dados
img_casos_diarios_aux = zeros(length(Estados),height(tabela)-indice_minimo+1);
img_casos_acumulados_aux = zeros(length(Estados),height(tabela)-indice_minimo+1);
img_obitos_diarios_aux = zeros(length(Estados),height(tabela)-indice_minimo+1);
img_obitos_acumulados_aux = zeros(length(Estados),height(tabela)-indice_minimo+1);

img_casos_diarios = zeros(length(Regioes),height(tabela)-indice_minimo+1);
img_casos_acumulados = zeros(length(Regioes),height(tabela)-indice_minimo+1);
img_obitos_diarios = zeros(length(Regioes),height(tabela)-indice_minimo+1);
img_obitos_acumulados = zeros(length(Regioes),height(tabela)-indice_minimo+1);

%Gerando matrizes para os 4: caso e obitos x diario e acumulados
for i=1:length(Estados)
    I_estado=find(arquivogeral.estado == string(Estados(i,:)));
    tabela_aux = arquivogeral(I_estado,:);
    tabela = tabela_aux(indice_minimo:height(tabela_aux),:);
    img_casos_diarios_aux(i,:) = tabela.casosNovos;
    img_obitos_diarios_aux(i,:) = tabela.obitosNovos;
    img_casos_acumulados_aux(i,:) = tabela.casosAcumulados;
    img_obitos_acumulados_aux(i,:) = tabela.obitosAcumulados;
end



for (i=1:length(Regioes))
if (i==1) init = 1; final = 7; end
if (i==2) init = 8; final = 16; end
if (i==3) init = 17; final = 20; end
if (i==4) init = 21; final = 23; end
if (i==5) init = 24; final = 27; end
for (j=init:1:final)
    img_casos_diarios(i,:) = img_casos_diarios(i,:)+img_casos_diarios_aux(j,:) ;
    img_obitos_diarios(i,:) = img_obitos_diarios(i,:)+img_obitos_diarios_aux(j,:)
    img_casos_acumulados(i,:) = img_casos_acumulados(i,:) +img_casos_acumulados_aux(j,:)
    img_obitos_acumulados(i,:) = img_obitos_acumulados(i,:) +img_obitos_acumulados_aux(j,:)
end
end
...
position = [round([0:size(imagem,1)/(size(img,1)):size(imagem,1)-(size(imagem,1)/(size(img,1)))] +(size(imagem,1)/(2*size(img,1))))];
    for (i=1:1:max(size(Regioes)));
        %text (-2,position(i),Estados(i));
        text (size(imagem,2)+50,position(i),Regioes(i));
    end
...
set(hfonte,'Rotation',90,'color',[0,0,0],'horizontalAlignment', 'right','verticalAlignment', 'top','FontSize',7);
    ...
    xticklabels({datestr(tabela.data(x_indices),20); num2str(round([0.0 , max(img_max)/2, max(img_max)]'),6) });    
xtickangle(45)
...
if (t == 2) title({'Novas mortes por semana',['Regiões do Brasil em ',datestr(tabela.data(length(tabela.data)),24)]},'FontSize',12); end
if (t == 1) title({'Novos casos por semana',['Regiões do Brasil em ',datestr(tabela.data(length(tabela.data)),24)]},'FontSize',12); end
if (t == 4) title({'Mortes acumuladas',['Regiões do Brasil em ',datestr(tabela.data(length(tabela.data)),24)]},'FontSize',12); end
if (t == 3) title({'Casos acumulados',['Regiões do Brasil em ',datestr(tabela.data(length(tabela.data)),24)]},'FontSize',12); end
...
end_time = tabela.data(length(tabela.data));
