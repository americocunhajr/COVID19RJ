## Como citar esses relatórios? / How to cite these reports? 

Esses relatórios são distribuidos segunda uma licença CC-BY-NC-ND 4.0.
These reports are released under a CC-BY-NC-ND 4.0 license.

```
@misc{COVID19RJ-Relatorio1,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 01 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 21$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (17/5/2020 até 23/5/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(24-05-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio1}},
}
```

```
@misc{COVID19RJ-Relatorio2,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 02 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 22$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (24/5/2020 até 30/5/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(31-05-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio2}},
}
```

```
@misc{COVID19RJ-Relatorio3,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 03 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 23$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (31/5/2020 até 06/6/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(07-06-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio3}},
}
```

```
@misc{COVID19RJ-Relatorio4,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 04 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 24$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (07/6/2020 até 13/6/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(14-06-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio4}},
}
```

```
@misc{COVID19RJ-Relatorio5,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 05 {P}or que é inadequado divulgar os indicadores de mortes por data de óbito? Uma explicação das diferentes metodologias para quantificar os óbitos durante a epidemia},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(19-06-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio5}},
}
```

```
@misc{COVID19RJ-Relatorio6,
   author = {L. {Lovisolo et al.}},
   title = {Relatório 06 {A} Mortalidade por {COVID-19} nos {E}stados {B}rasileiros: {U}ma análise comparativa entre 2019 e 2020 até o mês de maio},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(20-06-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio6}},
}
```

```
@misc{COVID19RJ-Relatorio7,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 07 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 25$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (14/6/2020 até 20/6/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(21-06-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio7}},
}
```

```
@misc{COVID19RJ-Relatorio8,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 08 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 26$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (21/6/2020 até 27/6/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(28-06-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio8}},
}
```

```
@misc{COVID19RJ-Relatorio9,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 09 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 27$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (28/6/2020 até 04/7/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(05-07-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio9}},
}
```

```
@misc{COVID19RJ-Relatorio10,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 10 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 28$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (05/7/2020 até 11/7/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(12-07-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio10}},
}
```

```
@misc{COVID19RJ-Relatorio11,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 11 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 29$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (12/7/2020 até 18/7/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(19-07-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio11}},
}
```

```
@misc{COVID19RJ-Relatorio12,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 12 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 30$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (19/7/2020 até 25/7/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(26-07-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio12}},
}
```

```
@misc{COVID19RJ-Relatorio13,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 13 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 31$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (26/7/2020 até 1/8/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(02-08-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio13}},
}
```

```
@misc{COVID19RJ-Relatorio14,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 14 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 32$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (02/8/2020 até 08/8/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(09-08-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio14}},
}
```

```
@misc{COVID19RJ-Relatorio15,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 15 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 33$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (09/8/2020 até 15/8/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(16-08-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio15}},
}
```

```
@misc{COVID19RJ-Relatorio16,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 16 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 34$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (16/8/2020 até 22/8/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(23-08-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio16}},
}
```

```
@misc{COVID19RJ-Relatorio17,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 17 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 35$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (23/8/2020 até 29/8/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(30-08-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio17}},
}
```

```
@misc{COVID19RJ-Relatorio18,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 18 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 36$^{a}$ à 39$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (30/8/2020 até 26/9/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(27-09-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio18}},
}
```

```
@misc{COVID19RJ-Relatorio19,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 19 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 40$^{a}$ à 43$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (26/9/2020 até 24/10/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(25-10-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio19}},
}
```

```
@misc{COVID19RJ-Relatorio20,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 20 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 44$^{a}$ à 48$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (26/10/2020 até 28/11/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(29-11-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio20}},
}
```

```
@misc{COVID19RJ-Relatorio21,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 21 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 49$^{a}$ à 52$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2020 (29/11/2020 até 26/12/2020)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(27-12-2020)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio21}},
}
```

```
@misc{COVID19RJ-Relatorio22,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 22 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 1$^{a}$ à 5$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (27/12/2020 até 30/01/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(31-01-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio22}},
}
```

```
@misc{COVID19RJ-Relatorio23,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 23 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 6$^{a}$ à 9$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (31/01/2021 até 27/02/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(28-02-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio23}},
}
```

```
@misc{COVID19RJ-Relatorio24,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 24 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 10$^{a}$ à 13$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (28/02/2021 até 28/03/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(28-03-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio24}},
}
```

```
@misc{COVID19RJ-Relatorio25,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 25 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 14$^{a}$ à 18$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (29/03/2021 até 01/05/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(02-05-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio25}},
}
```

```
@misc{COVID19RJ-Relatorio26,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 26 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 19$^{a}$ à 22$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (02/05/2021 até 29/05/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(30-05-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio26}},
}
```

```
@misc{COVID19RJ-Relatorio27,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 27 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 23$^{a}$ à 26$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (30/05/2021 até 26/06/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(27-06-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio27}},
}
```

```
@misc{COVID19RJ-Relatorio28,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 28 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 27$^{a}$ à 31$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (27/06/2021 até 31/07/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(02-08-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio28}},
}
```

```
@misc{COVID19RJ-Relatorio29,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 29 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 32$^{a}$ à 35$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (01/08/2021 até 28/08/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(29-08-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio28}},
}
```

```
@misc{COVID19RJ-Relatorio30,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 30 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 36$^{a}$ à 40$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (29/08/2021 até 02/10/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(03-10-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio28}},
}
```

```
@misc{COVID19RJ-Relatorio31,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 31 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 41$^{a}$ à 44$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (03/10/2021 até 30/10/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(31-10-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio28}},
}
```

```
@misc{COVID19RJ-Relatorio32,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 32 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 45$^{a}$ à 49$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (31/10/2021 até 04/12/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(05-12-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio28}},
}
```

```
@misc{COVID19RJ-Relatorio33,
   author = {A. {Cunha~Jr et al.}},
   title = {Relatório 33 {P}rogresso da {COVID-19} no {B}rasil e no {E}stado do {R}io de {J}aneiro 50$^{a}$ à 52$^{a}$ {S}emana {E}pidemiológica do {C}alendário 2021 (05/12/2021 até 25/12/2021)},
   howpublished = {COVID-19: Observatório Fluminense},
   year = {(12-26-2021)},
   note = {\url{https://https://doi.org/10.12957/eduerj.covid19rj.relatorio28}},
}
```
